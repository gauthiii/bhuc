...
| AG-4 | Build **Consent & Data Protection Agent** (UC3/P4) — Part 2 labeler. **Tools built + verified over A2A 2026-07-08:** Tool A "Detect and Tag Part 2/SUD content" (script, keyword classifier) works; Tool B was a CRUD **Create** on `u_bhuc_consent` (inserted junk rows, never touched the note table) — **replaced with a custom Script tool** that **updates** BOTH `u_bhuc_care_plan` (`u_sensitivity`+`u_contains_part2`) and the patient's `part2_sud` consent (`u_sensitivity`+`u_labeled_by_agent`). Re-verified end-to-end: care_plan flagged part2, no junk inserts. **Still open:** detect-tool negation fix (`"no substance use"` → false-positive part2; fixed script provided, pending paste) + SN-4/SN-13 ACL enforcement (deny-by-default is not yet enforced anywhere). | §4.4 Agent 4 | L | 2 | AG-0 | ◐ |
