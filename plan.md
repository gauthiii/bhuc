...
- **G-1 — Exact Zurich build number.** ACL-denied on `glide.buildtag`. Presence of Zurich-only tables (`sn_ai_governance_mcp_server_sync`, the `AICT Enterprise AI Dataset assets usage` OneExtend capability) is consistent with Zurich, but the precise build string must be read from **System Diagnostics → Stats** while logged into the UI.
- **G-2 — Dashboard tab layout.** The nine AICT dashboard "tabs" are UX Framework workspace surfaces, not tables; the REST API can confirm the **data tables** behind them exist (it does) but cannot confirm the rendered tab configuration. Confirm by opening `Workspaces → AI Control Tower` in the UI.
