...
- **Scripted demo examples (FE-9f):** each groundable agent has a 3rd example chip that plays a deliberately-ungrounded reply after a 3–5s think-time (Risk → "Score MADRS"; Clinical Doc → "Code an ADHD eval") — both cite instruments/codes/policies outside the agent's KB. `AgentChat.send()` carries an optional `canned` reply that bypasses the A2A relay; the hallucination check then runs *for real* on that text and lands **~28% / ~32%** grounded (both < 35% → "possible hallucination"), giving a repeatable on-page demo of the detector firing. The "Live · A2A" badge and the "Use Case 1/2" card subtitles were commented out on this page.
...
- **Purpose:** Administer validated instruments **one at a time**; on submit, trigger the BHUC Risk Identification Agent for scoring.

> **As-built — SUD battery expansion (2026-07-10).** Screening grew from 3 instruments to **10** on an **adaptive SBIRT model**. Every patient completes the mental-health spine (**C-SSRS → PHQ-9 → GAD-7**) then the **NIDA Quick Screen** (SUD gateway); the substance-use instruments branch off it: alcohol-positive → **AUDIT**, drug-positive → **DAST-10**; AUDIT ≥ 8 → **Craving & Triggers** (custom module), DAST-10 ≥ 3 → **SOWS**; either positive → **BAM** + **SOCRATES** (8A alcohol / 8D drug wording). Branching is administration logic only (client `lib/screeningFlow.ts` + server `risk.py next_instrument`); scoring stays server-side. All SUD instruments are 42 CFR Part 2 data (rows flagged `part2_sud`). SOCRATES/BAM are scored by **subscale** (no single total) — computed server-side and stored in `u_bhuc_screening.u_subscores`. Instrument scoring rules for all 10 live in the Screening Scoring Rules KB (§4.4 Agent 2). See `agents/agent2_sud_update.md` + `tables.md` Table 2.
...
| SUD-battery options | radio (per-instrument scales) | NIDA freq (Never…Daily); AUDIT freq/qty/yes-no; DAST-10 Yes/No (item 3 reverse); Craving 0–4; SOWS 0–4; BAM mixed day/intensity; SOCRATES 1–5 Likert | numeric value stored; instrument unlocked by branching | default/selected/focus |
...
- **Inputs & validation:**
  ...
  - Instrument order enforced: C-SSRS (when required) before PHQ-9/GAD-7 unlock, then the NIDA gateway; SUD instruments unlock adaptively per the branching rules above (a low-risk patient answers only the 4-instrument spine).
...
> **As-built (2026-07-10) — prior-auth packet realized as an editable clinical-document.** The C6 Prior-Auth card is built as a full sample-style **prior-authorization document** (letterhead + sections: Request Summary, Member Information, Referring Provider, Clinical Justification, Coverage Determination, Treatment Plan & Goals, SUD Detail, Payer Use Only) with an **eye toggle** for a clean PDF-style preview, a **diagnosis dropdown** sourced from ICD-10 codes suggested across the patient's signed notes, and per-field **42 CFR Part 2 redaction** (black bars) enforced server-side in `priorauth.py` (`_PART2_FIELDS`) so it holds in edit AND preview. **Agent 5 produces ~6 values** (service, primary dx, requested units, cited coverage determination + citation, SUD detail); the other document fields were clinician-blank. **The 12-field clinician gap is now CLOSED (option-b "reuse existing data"):** `_build_document` seeds all 12 as editable pre-fills from data we already have — service→CPT/HCPCS + ASAM Level of Care via a coding map (IOP→H0015/ASAM 2.1, PHP→H0035/2.5, MAT→H0033, Residential→H0018, Outpatient→H0004), Requested Start = today, Authorization Period parsed from the units duration; Presenting problem / Measurable goals reused from the latest **signed** note's Chief-Complaint/HPI/Plan sections + care-plan next-steps; ASAM dimension summary + medical-necessity rationale synthesized from the screening bands. No Agent 5 change and no new A2A call; SUD-revealing seeds (incl. `level_of_care`) stay behind the same redaction. See action item FE-9m and memory `bhuc-priorauth-fields`.
...
> **As-built — SUD battery expansion (2026-07-10, applied + verified over A2A).** Agent 2 now scores **10 instruments**: the spine (C-SSRS, PHQ-9, GAD-7) plus the substance-use battery (NIDA Quick Screen, AUDIT, DAST-10, Craving & Triggers, SOWS, BAM, SOCRATES). Changes made live: **description/role/instructions** rewritten (below); **Tool A** results limit **10 → 15** (KB grew 5 → 12 articles); **Tool B** gained a 5th input **`subscores`** (writes `u_bhuc_screening.u_subscores`) and the live script now uses **`GlideRecordSecure`**. Summed instruments map a total → severity band → BHUC band; subscale instruments (BAM, SOCRATES) are scored **server-side** (`risk.py compute_subscores`, written at record creation and passed to the agent as an authoritative block it persists verbatim — the LLM computed subscales inconsistently). All 7 SUD instruments carry `part2_sud`. The 10 `u_instrument` choices are provisioned in `sys_choice` (else non-spine values are blanked on write). Full change spec + verified A2A tests in **`agents/agent2_sud_update.md`**; KB articles in `knowledge/bhuc-screening-scoring-rules.md`.

**Purpose:** Scores validated instrument responses (C-SSRS/PHQ-9/GAD-7 + the SUD battery: NIDA/AUDIT/DAST-10/Craving/SOWS/BAM/SOCRATES) into a risk band + confidence with a rationale (and per-subscale detail for BAM/SOCRATES), and routes the draft to a clinician — always deferring the final determination (Supervised).

**Step 1 — Define specialty.**
- **Name:** `BHUC Risk Identification Agent`
- **AI agent description:** *(current live text — see `agents/agent2_sud_update.md` for the canonical copy)*
  > You score behavioral-health screening instruments into a risk band (Low, Moderate, High) with a confidence value and a short rationale that lists the specific responses that drove the score. You handle the mental-health spine (C-SSRS, PHQ-9, GAD-7) and the substance-use battery (NIDA Quick Screen, AUDIT, DAST-10, the Craving & Triggers module, SOWS, BAM, and SOCRATES), scoring exactly one instrument per request. For summed instruments you compute the total and map it to the published band; for subscale instruments (BAM, SOCRATES) you compute each subscale and never invent a single total. You retrieve the matching scoring rules from the Screening Scoring Rules knowledge base and score only from those rules, never from memory. Substance-use instruments contain 42 CFR Part 2 (SUD) information; you keep patient identifiers out of the free-text rationale. You never make a final clinical determination — every score is a draft that a licensed triage clinician must confirm, adjust, or reject.
- **AI agent role:**
  > Real-time triage risk-scoring decision support across the mental-health and substance-use screening battery. Produces a risk band + confidence + rationale (and subscale detail for BAM/SOCRATES) for clinician confirmation. Runs as the invoking clinician's identity. SUD screening rows are 42 CFR Part 2 data.
- **List of steps:**
  1. Look up the screening record and its instrument responses.
  2. Use Search Retrieval to load the scoring rules for the relevant instrument (query by instrument name) + the risk-banding article.
  3. Compute the risk band + confidence and a rationale citing the driving responses (summed instruments → total→band; subscale instruments → per-subscale, using the server-supplied "Precomputed subscores").
  4. Write the draft score via the write-back Script tool; for BAM/SOCRATES pass `subscores` verbatim.
...
- **Tool A — Search retrieval (scoring rules). [As-built: `AIA RAG Retriever` → profile `BHUC Screening Search`]** Built per §4.6.2 over KB `BHUC Screening Scoring Rules` (`532b483f…`, now 12 articles). Hybrid; Results limit **`15`** (raised from 10 for the SUD battery); threshold `0.3`. **Autonomous.**
- **Tool B — Write-back Script (NOT a Record Operation as-built). [As-built: `Write risk score (script)`]** A Script tool is what actually worked. Declared inputs: **`risk_band`, `confidence`, `rationale`, `screening_sys_id`, and `subscores`** — the target record's **sys_id must be an input**, because the agent has no record-lookup tool to resolve a Number to a sys_id (§4.6.4). Body does `GlideRecordSecure('u_bhuc_screening').get(inputs.screening_sys_id)`, sets `u_risk_band/u_confidence/u_rationale/u_scored_by_agent=true/u_state='scored'`, writes `u_subscores` when `subscores` is present (BAM/SOCRATES), and **`return`s** a JSON string — it must NOT use `outputs` (§4.6.1). **Autonomous.** *(Now uses `GlideRecordSecure` — ACL-gated under `svc-bhuc-risk-ai`, live 2026-07-09.)*
...
> **As-built — screening-driven documentation (2026-07-10, applied + verified over A2A).** The draft source changed from a hardcoded canned encounter to the patient's **actual latest screening results — one per instrument, deduped** (prefer clinician-confirmed, else latest scored), synthesized across instruments and mapped to candidate ICD-10 per the coding KB. **KB fix:** 4 of 5 Clinical Coding articles (KB0010018/19/20/21) held router help-text placeholders — re-pushed real content from the mirror; expanded ICD-10 (SUD F10–F19) + CPT (screening/SBIRT); added KB0010046 (interpretation/ICD-10 map), KB0010047 (multi-instrument synthesis), KB0010048 (screening-to-narrative template) as Drafts to publish. **Backend** (`note.py`): `_screening_summary`/`_draft_from_screenings` + new field `u_bhuc_care_plan.u_source_screenings` for traceability. **Agent 3 edits** (description/role/instructions) in `agents/agent3_screening_update.md`. Verified: `BHUC_CARE_PLAN_024` synthesized 5 instruments → codes R45.851/F32.1/F41.1/F10.20/F19.20 + CPT 90791.

**Purpose:** Documentation drafter. It synthesizes the patient's latest screening battery (one result per instrument) into a draft clinical note, grounds every line to a screening result, flags unverified lines, and suggests ICD-10/CPT codes — but never signs. A clinician edits and signs (Supervised, screen C5). *(Originally specced as an ambient scribe over a live transcript; the as-built source is the structured screening summary — see the 2026-07-10 note above.)*
